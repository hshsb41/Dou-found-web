...
<head>
    ...
    <style>
        ...
        .btn-ios:active { transform: scale(0.95); opacity: 0.9; }
        ...
        .data-label { color: rgba(255,255,255,0.4); font-weight: 700; font-size: 10px; text-transform: uppercase; letter-spacing: 1px; }
...
<body>
    <div class="glass-card">
        <div class="text-center mb-10">
            <h1 class="text-3xl font-black tracking-tighter text-white">CKR <span class="text-sky-500">UNKNOWN</span></h1>
            <p class="text-[10px] text-gray-500 font-bold uppercase tracking-[4px] mt-1">Duo Extractor</p>
        </div>

        <div class="space-y-5">
            <input type="text" id="uid" placeholder="PLAYER UID" class="input-field w-full py-5 text-xl tracking-widest">
            <button onclick="fetchData()" id="btn" class="btn-ios w-full py-5 text-xs">View Details</button>
        </div>

        <div id="outputContainer" class="terminal mt-8 p-6 border border-white/5">
            <div id="loader" class="text-center py-2 text-sky-400 font-bold text-[10px] animate-pulse">CONNECTING TO SERVER...</div>
            <div id="result" class="hidden space-y-4">
                </div>
        </div>

        <div class="mt-10 text-center">
            <p class="text-[10px] text-gray-600 font-black tracking-widest">DEVELOPER : CKRPRO DEV</p>
        </div>
    </div>
                ...
                if(res.status === 'success') {
                    resultBox.innerHTML = `
                        ...
                        <div class="pt-1">
                            ...
                            <div class="data-value text-sky-400 text-[11px] mt-1">${res.data.time_created}</div>
                        </div>
                    `;
                } else {
                    resultBox.innerHTML = `<div class="text-red-500 text-[11px] font-bold text-center uppercase">[!] ${res.message}</div>`;
                }
            } catch (err) {
                loader.classList.add('hidden');
                resultBox.classList.remove('hidden');
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            return jsonify({
                "status": "success", 
                "data": {
                    "partner_uid": str(d.partner_uid), 
                    "level": lv, 
                    "score": s, 
                    "days_active": d.days_active,
                    "time_created": format_ts(d.creation_timestamp)
                }
            })
